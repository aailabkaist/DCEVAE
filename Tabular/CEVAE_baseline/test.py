...
def test(test_loader, args, logger):
    ...
    with torch.no_grad():
        ...
        line = '{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(np.sum(ys) / ys.shape[0],\
               np.sum(o1s) / o1s.shape[0], np.sum(o2s) / o2s.shape[0], np.sum(o3s) / o3s.shape[0], np.sum(o4s) / o4s.shape[0])
...
def generate_data(loader, args, dataset='train'):
    ...
    with torch.no_grad():
        for idx, (r, d, a, y) in enumerate(loader):
            ...
            r_hard, d_hard, y_hard, d_hard_cf, y_cf_hard = model.reconstruct_hard(u_prev, a.to(device))
            i_hard = torch.cat((r_hard, d_hard), 1)
            i_cf_hard = torch.cat((r_hard, d_hard_cf), 1)
