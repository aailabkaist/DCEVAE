...
class CelebA(data.Dataset):
        ...
        for attr_path in attr_path_list:
            ...
            for line in rdr:
                if temp == 0:
                    ...
                    if small == True:
                        if 'train' in attr_path and num_tr < train_num:
                            self.train_dataset.append([filename, label, rest, des])
                            num_tr += 1
                        elif 'valid' in attr_path and num_va < valid_num:
                            self.valid_dataset.append([filename, label, rest, des])
                            num_va += 1
                        elif 'test' in attr_path and num_te < test_num:
    ...
    def __getitem__(self, index):
        ...
        filename, label, rest, des = dataset[index]
